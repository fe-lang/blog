...
At its core, Fe is still a modern statically typed, EVM-native language. It has
Rust-like syntax, data types, generics, traits, pattern matching, and
compile-time code execution. It has a simplified borrowing model, with no
lifetime annotations, but with explicit control over mutability, ownership, and
borrowing. The language is now expressive enough for much of the functionality
that was previously built into the compiler to be implemented in the core and
std libraries, e.g. Solidity compatibility.
...
To be clear, parts of this plan aren't yet fully realized or need
refinement. More details on this below.
...
the 0.X numbering scheme from 2023, in favor of `<year>.<minor>.<patch>`; 1.0
...
Single-Assignment) form, a well-established compiler intermediate representation
used by LLVM and other major compiler frameworks. This allows us to benefit from
a wealth of compiler optimization research and prior art. Sonatina IR is
relatively high-level, with rich struct, array, and int size type information
...
spilling. There's no stack-too-deep, and there's no limitation on call graph
...
Fe 26.0 ships with an integrated toolchain, similar to the Rust tooling (though
not yet as polished). Fe projects are organized similarly to Rust projects, with
...
compiler. It has bugs, the standard library is limited, and it generates sub-par
EVM bytecode in many cases. Testing support is limited, and debugging support is
nonexistent. Integration with ecosystem tools like Foundry and Hardhat is
missing. In some places, the flexibility and power we plan to support isn't
fully unlocked.

We'll be releasing regularly going forward. Short-term focus areas are:

- Compiler hardening via fuzzing, additional test cases, a suite of
  cross-language comparisons to ensure correct behavior and decent performance.
- Improved EVM bytecode generation. There's a lot of low-hanging fruit here.
...
There will likely be breaking changes in the near future. As the language and compiler
mature, we'll decide on and communicate stability guarantees.
...
friendly competition, via projects like a community "benchmark game"-style
comparative benchmarking suite, where we can collectively learn which compilers
are optimizing which patterns well, and we can share ideas for the benefit of
the entire ecosystem. We also aim to make Sonatina an useful backend option
